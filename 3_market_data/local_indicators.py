...
def _fetch_ohlcv(symbol: str, curr_date: str, look_back_days: int) -> pd.DataFrame:
    ...
    start = end - timedelta(days=look_back_days + 50)
...
def _format_summary(symbol: str, df: pd.DataFrame, curr_date: str) -> str:
    ...
    if pd.notna(bb_u) and pd.notna(bb_l):
        ...
        if close_price >= bb_u:   bb_pos = "AT/ABOVE upper band"
        elif close_price <= bb_l: bb_pos = "AT/BELOW lower band"
    ...
    if pd.notna(sma50) and pd.notna(sma200):
        trend = "BULLISH (Golden Cross)" if sma50 > sma200 else "BEARISH (Death Cross)"
    else:
        trend = "N/A"

    summary = f"""
=== LOCAL TECHNICAL ANALYSIS: {symbol} as of {curr_date} ===
[Computed locally via pandas-ta]

PRICE ACTION
  Close  : ${close_price:.2f}  Change: ${price_change:+.2f} ({pct_change:+.2f}%)
  High   : ${last['high']:.2f}   Low: ${last['low']:.2f}
  Volume : {int(last['volume']):,}

TREND
  SMA 50 : {v('sma_50')}  |  SMA 200: {v('sma_200')}
  EMA 10 : {v('ema_10')}  |  Trend  : {trend}

MOMENTUM
  RSI 14 : {v('rsi_14')} → {rsi_signal}
  MACD   : {v('macd')}  |  Signal: {v('macd_signal')}  |  Hist: {v('macd_hist')}
  MACD Direction: {macd_cross}

VOLATILITY
  ATR 14 : {v('atr_14')}
  BB Upper: {v('bb_upper')}  |  Mid: {v('bb_mid')}  |  Lower: {v('bb_lower')}
  BB Position: {bb_pos}
